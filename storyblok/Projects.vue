    ...
    <div class="container row projects__wrapper">
      ...
      <div class="projects__col">
        ...
        <div
          ...
          v-for="(item, index) in blok.your_work"
          :key="item._uid"
          :class="{
            'projects__content--is-active': index === currentActiveSlide,
          }"
        >
          <span class="projects__highlight-text">{{
            item.highlight_text
          }}</span>
        ...
        <div class="projects__control-panel" v-if="blok.your_work.length > 1">
          <button class="projects__controller" @click="handleSlideChange(-1)">
            Prev
          </button>
          <button class="projects__controller" @click="handleSlideChange(1)">
            Next
        ...
        <div class="projects__showcase">
          ...
          <div
            class="projects__showcase-inner"
            v-for="(item, index) in blok.your_work"
            :key="item._uid"
          >
            <video
              ...
              muted
              loop
              v-if="isVideo(item.image_secondary.filename) === true"
              :class="{
                'projects__video--is-active': index === currentActiveSlide,
              }"
            >
              <source :src="item.image_secondary.filename" type="video/mp4" />
            </video>
            <img
              class="projects__secondary-image"
              :class="{
                'projects__secondary-image--is-active':
                  index === currentActiveSlide,
              }"
              :src="item.image_secondary.filename"
              v-else
            />
          </div>
        </div>
      ...
      <div class="projects__col">
        <div class="projects__image-wrap">
          ...
          <img
            ...
            :class="{
              'projects__image--is-active': index === currentActiveSlide,
            }"
          />
        </div>
      </div>
